...
The commands `slurm-quota user-quota`, `slurm-quota account-quota`, `slurm-quota user-gpu-quota`, and `slurm-quota account-gpu-quota` respectively allow assigning CPU and GPU quotas to users and accounts through the REST API (operator or admin role required; see below).
The `slurm-quota adjust` command allows manually adjusting consumed CPU/GPU time for one user or account with an explicitly signed delta through the REST API (operator or admin role required; see below).

Default quotas used when the solution auto-creates a user or account can be displayed with `slurm-quota default-quotas` and updated with `slurm-quota set-default-quotas` through the REST API (operator or admin role required; see below). These defaults are applied only to newly auto-created entries and do not modify existing users/accounts.
...
REST API authorization uses four roles. **Admin** users are listed in `[authorization] admins` in `serve.ini` (bootstrap only; not stored in the database). **Operator** users are stored in the SQLite `operators` table and can view all statistics and manage quotas, consumption, default quotas, and GPU factors. **Manager** users are stored in the `managers` table with account assignments in `manager_accounts`; they can view statistics only for assigned accounts and users who belong to those accounts (via Slurm). All other authenticated users have the **user** role and can view only their own consumption stats (including Slurm accounts they belong to). Admins can grant or revoke operator and manager roles through `GET /roles`, `PUT /roles/operators/<username>`, `DELETE /roles/operators/<username>`, `PUT /roles/managers/<username>`, and `DELETE /roles/managers/<username>`. Admins assign accounts to managers with `GET /roles/managers/<username>/accounts`, `PUT /roles/managers/<username>/accounts/<account>`, and `DELETE /roles/managers/<username>/accounts/<account>`. The `slurm-quota role` CLI subcommands and the web dashboard **Manage roles** page (admin only) call these endpoints. Operators and admins can set user and account CPU/GPU quotas through `PUT /quotas/users/<username>/cpu`, `PUT /quotas/users/<username>/gpu`, `PUT /quotas/accounts/<account>/cpu`, and `PUT /quotas/accounts/<account>/gpu` (JSON body: `{"quota_minutes": <int>}`, where `-1` means unlimited). The `slurm-quota user-quota`, `account-quota`, `user-gpu-quota`, and `account-gpu-quota` CLI subcommands and the web dashboard quota edit forms call these endpoints. Operators and admins can view and update default quotas applied to newly auto-created users/accounts through `GET /quotas/defaults` and `PUT /quotas/defaults` (JSON body: partial object with any of `user_cpu_minutes`, `user_gpu_minutes`, `account_cpu_minutes`, `account_gpu_minutes`; `-1` means unlimited). The `slurm-quota default-quotas` and `set-default-quotas` CLI subcommands call these endpoints. Operators and admins can view and update GPU charging factors through `GET /factors/gpu` (response: `{"default_factor": <float>, "factors": {<gpu_type>: <float>, ...}}`) and `PUT /factors/gpu/<gpu_type>` (JSON body: `{"factor": <positive float>}`). The `slurm-quota gpu-factors` and `set-gpu-factor` CLI subcommands call these endpoints. Operators and admins can adjust consumed CPU/GPU time through `PATCH /consumption/user/<username>/cpu`, `PATCH /consumption/user/<username>/gpu`, `PATCH /consumption/account/<account>/cpu`, and `PATCH /consumption/account/<account>/gpu` (JSON body: `{"delta_minutes": <signed int>}`; response: `{"total_consumed_minutes": <int>}`). The `slurm-quota adjust` CLI subcommand and the web dashboard consumption edit forms call these endpoints.
...
The `slurm-quota-web` application is a web dashboard that retrieves the same statistics from the HTTP API (`GET /stats`) and renders them as HTML tables and quota usage bars. When `authentication.method=ldap`, the dashboard presents a login page and stores each user's API JWT in a signed, HttpOnly session cookie. Alternatively, set `SLURM_QUOTA_TOKEN` in the web server environment to authenticate API calls with a service token (no per-user login). Users with the **user** role see only their own stats; managers see stats for their assigned accounts; operators and admins see all data. Operators and admins can edit quotas and adjust consumption inline on the dashboard. Admins can open **Manage roles** to list all users with their role, grant or revoke operator and manager access, and assign accounts to managers. It can run standalone with Flask built-in HTTP server for local testing, or be launched by a production-ready HTTP server (for example Apache with mod_wsgi) as a WSGI application.
...
3) Configure the web dashboard environment:

Create `/etc/default/slurm-quota-web`, to set at least `SLURM_QUOTA_URL`; when LDAP browser login is used, also set `SLURM_QUOTA_WEB_SESSION_KEY_FILE`:
...
4) Create the session signing key file (skip when using `authentication.method=jwt`):
...
> [!NOTE]
> As an alternative for quick testing, you can set `SLURM_QUOTA_WEB_SESSION_KEY` directly in `/etc/default/slurm-quota-web`. This is less suitable for production than a key file.
...
When `authentication.method=ldap`, users sign in through the web dashboard with their LDAP credentials. Alternatively, set `SLURM_QUOTA_TOKEN` in `/etc/default/slurm-quota-web` for service-account access without a login page. Behind HTTPS, set `SLURM_QUOTA_WEB_SECURE_COOKIES=1` in the same file so cookies are marked `Secure`. Session lifetime defaults to one day (`SLURM_QUOTA_WEB_SESSION_DAYS`) to match the default JWT duration.
...
Enable and reload Apache as shown in the RPM installation section.
